...
public class PerspectiveRenderer extends EntityRenderer {
    ...
    public PerspectiveRenderer(PerspectiveMod mod, Minecraft mc, IResourceManager resourceManager) {
    ...
    public void loadShader(ResourceLocation resourceLocationIn)
    {
        ...
        {
            this.mod.logger.warn("Failed to load shader: {}", resourceLocationIn, ioexception);
            this.shaderIndex = SHADER_COUNT;
            this.useShader = false;
        }
        catch (JsonSyntaxException jsonsyntaxexception)
        {
            this.mod.logger.warn("Failed to load shader: {}", resourceLocationIn, jsonsyntaxexception);
            this.shaderIndex = SHADER_COUNT;
            this.useShader = false;
        }
    }
    ...
    private void createWorldIcon()
    {
        if (this.mc.renderGlobal.hasNoChunkUpdates() && !this.mc.getIntegratedServer().isWorldIconSet())
        {
            ...
            {
                this.mod.logger.warn("Couldn't save auto screenshot", (Throwable)ioexception);
            }
        }
    }
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                if (this.mod.cameraToggled) {
                    GlStateManager.rotate(this.mod.cameraPitch - f2, 1.0F, 0.0F, 0.0F);
                    GlStateManager.rotate(this.mod.cameraYaw - f1, 0.0F, 1.0F, 0.0F);
                    GlStateManager.translate(0.0F, 0.0F, (float) (-d3));
                    GlStateManager.rotate(f1 - this.mod.cameraYaw, 0.0F, 1.0F, 0.0F);
                    GlStateManager.rotate(f2 - this.mod.cameraPitch, 1.0F, 0.0F, 0.0F);
                } else {
        ...
        if (!this.mc.gameSettings.debugCamEnable)
        {
            ...
            if (this.mod.cameraToggled) {
                GlStateManager.rotate(this.mod.cameraPitch, 1.0f, 0.0f, 0.0f);
                GlStateManager.rotate(this.mod.cameraYaw + 180.0f, 0.0f, 1.0f, 0.0f);
            }else {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        if (this.mc.inGameHasFocus && flag)
        {
            ...
            if (this.mc.gameSettings.smoothCamera)
            {
                ...
                if (this.mod.cameraToggled) {
                    final PerspectiveMod mod = this.mod;
                    mod.cameraYaw += f3 / 8.0f;
                    final PerspectiveMod mod2 = this.mod;
                    mod2.cameraPitch += f4 / 8.0f;
            ...
            else if (this.mod.cameraToggled) {
                final PerspectiveMod mod3 = this.mod;
                mod3.cameraYaw += f2 / 8.0f;
                final PerspectiveMod mod4 = this.mod;
                mod4.cameraPitch += f3 / 8.0f;
                if (Math.abs(this.mod.cameraPitch) > 90.0f) {
                    this.mod.cameraPitch = ((this.mod.cameraPitch > 0.0f) ? 90.0f : -90.0f);
                }
            }
            ...
            if (this.mc.currentScreen != null)
            {
                ...
                catch (Throwable throwable)
                {
                    ...
                    crashreportcategory.addDetail("Screen name", new ICrashReportDetail<String>()
                    {
                        ...
                        {
                            return PerspectiveRenderer.this.mc.currentScreen.getClass().getCanonicalName();
                        }
                    });
                    ...
                    crashreportcategory.addDetail("Screen size", new ICrashReportDetail<String>()
                    {
                        public String call() throws Exception
                        {
                            return String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(), PerspectiveRenderer.this.mc.displayWidth, PerspectiveRenderer.this.mc.displayHeight, scaledresolution.getScaleFactor());
                        }
                    });
